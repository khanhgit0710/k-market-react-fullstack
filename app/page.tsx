...
export default async function Home({
  ...
  return (
    <div className="bg-[#f5f5f5] min-h-screen flex flex-col font-sans text-gray-800">
      <Header />
      <main className="w-full max-w-[1200px] mx-auto py-3 md:py-8 flex items-start gap-0 md:gap-5 flex-grow px-2 md:px-4 overflow-x-hidden">
        ...
        {/* List Hàng: THÊM items-start VÀO GRID ĐỂ CARD KHÔNG BỊ GIÃN */}
        <div className="flex-1 min-w-0">
          <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-2.5 items-start">
            {products.map((item: any) => (
              <Link href={`/product/${item._id}`} key={item._id} className="group block h-full">
                <div className="bg-white rounded-sm shadow-sm hover:shadow-md transition-all duration-300 border border-transparent hover:border-orange-500 flex flex-col h-full overflow-hidden">

                  {/* KHUNG ẢNH: Dùng h-48 hoặc aspect-square để nhốt cái ảnh lại */}
                  ...
                  <div className="p-2.5 flex flex-col flex-1 bg-white">
                    <h4 className="text-[12px] line-clamp-2 leading-4 h-8 text-gray-700 group-hover:text-orange-600 transition-colors mb-2">
                      {item.name}
                    </h4>
                    <div className="mt-auto">
                      ...
                      <div className="flex justify-between items-center mt-1">
                        ...
                        <p className="hidden sm:block text-[10px] text-gray-400">{item.location}</p>
                      </div>
                    </div>
                  </div>
